...
describe('PermissionGate', () => {
  ...
  it('asks for commands and sensitive files', async () => {
    ...
    await gate.authorize({ tool: 'run_command', kind: 'shell', detail: 'npm test' });
  ...
  it('denies prompts when no interactive approver is available', async () => {
    ...
    await expect(
      gate.authorize({ tool: 'run_command', kind: 'shell', detail: 'npm test' }),
  ...
  it('enforces plan mode and bypass mode', async () => {
    ...
    await expect(
      bypass.authorize({ tool: 'run_command', kind: 'shell', detail: 'anything' }),
    ).resolves.toBeUndefined();
  });
});
