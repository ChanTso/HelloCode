...
import { AnthropicModel, DEFAULT_MODEL, formatProviderError } from './anthropic.js';
...
function parseCli(argv: string[]) {
  ...
  if (parsed.values.help === true) {
    return { action: 'help' as const, values: parsed.values, positionals: parsed.positionals };
  }
  if (parsed.values.version === true) {
    return { action: 'version' as const, values: parsed.values, positionals: parsed.positionals };
  }
  return { action: 'run' as const, values: parsed.values, positionals: parsed.positionals };
}
...
async function resolveConfig(
  ...
  if (
    ...
    throw new Error('--plan and --dangerously-skip-permissions cannot be combined.');
  }
  ...
  const maxTurns = parsePositiveInteger(values['max-turns'] ?? '40', '--max-turns', 200);
  ...
  return {
    ...
    interactive,
    maxTurns,
    mode: values['dangerously-skip-permissions'] === true
      ? 'bypass'
      : values.plan === true
        ? 'plan'
        : 'default',
...
async function run(config: CliConfig, ui: TerminalUI): Promise<number> {
  ...
  const model = new AnthropicModel({ apiKey: config.apiKey, model: config.model });
...
async function interactiveLoop(
  ...
  while (!exitRequested) {
    ...
    try {
      ...
      if (exitRequested || isAbortError(error) || isClosedReadlineError(error)) break;
      throw error;
    }
...
function reportStop(result: AgentRunResult, ui: TerminalUI): void {
  switch (result.stop) {
    ...
    case 'max_tokens':
      ...
      return;
    case 'context_limit':
      ui.notice('The model context window was exhausted. Use /compact or /clear.');
      return;
    case 'refusal':
...
function isClosedReadlineError(error: unknown): boolean {
  return error instanceof Error && 'code' in error && (error as NodeJS.ErrnoException).code === 'ERR_USE_AFTER_CLOSE';
}
