...
import type { ApprovalPrompt, PermissionMode, PermissionRequest } from './permissions.js';
...
export class TerminalUI {
  ...
  readonly approve: ApprovalPrompt = async (
    ...
    this.finishText();
    const label = request.sensitive === true ? 'sensitive access' : request.kind;
  ...
  error(message: string): void {
    this.finishText();
    this.#error.write(`${this.#red('error:')} ${sanitizeTerminalText(message)}\n`);
  }

  finishText(): void {
    if (this.#wroteText && !this.#textEndedWithNewline) this.#output.write('\n');
