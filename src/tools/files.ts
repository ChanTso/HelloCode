...
export function createFileTools(): ToolSpec[] {
  return [readFileTool, listFilesTool, searchTextTool, editFileTool, writeFileTool];
}
...
const readFileTool = defineTool<ReadInput>({
  definition: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        limit: { type: 'integer', minimum: 1, maximum: 2000, description: 'Maximum lines to return. Defaults to 400.' },
      },
      required: ['path'],
  ...
  parse(input) {
    ...
    return {
      ...
      offset: optionalIntegerField(value, 'offset', 1, Number.MAX_SAFE_INTEGER) ?? 1,
  ...
  async execute(input, context) {
    ...
    if (start >= lines.length) {
      throw new Error(`Offset ${input.offset} is past the end of ${input.path} (${lines.length} lines).`);
    }
    const selected = lines.slice(start, start + input.limit);
    const body = selected
      .map((line, index) => `${String(input.offset + index).padStart(6)}\t${line}`)
      .join('\n');
    const lastLine = input.offset + selected.length - 1;
    const continuation = lastLine < lines.length
  ...
  definition: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        pattern: { type: 'string', description: 'Glob matched relative to path. Defaults to **/*.' },
      },
      additionalProperties: false,
    },
  },
  ...
  definition: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        pattern: { type: 'string', description: 'Optional file glob, such as **/*.ts.' },
...
const editFileTool = defineTool<EditInput>({
  ...
  parse(input) {
    const value = objectInput(input, ['path', 'old_text', 'new_text', 'replace_all']);
    return {
      path: stringField(value, 'path'),
      oldText: stringField(value, 'old_text'),
      newText: typeof value.new_text === 'string'
        ? value.new_text
  ...
  async execute(input, context) {
    ...
    if (!fileStat.isFile()) throw new Error(`Not a file: ${input.path}`);
    if (fileStat.size > MAX_FILE_BYTES) throw new Error('File is too large to edit.');
...
const writeFileTool = defineTool<WriteInput>({
  ...
  parse(input) {
    ...
    if (Buffer.byteLength(value.content) > MAX_WRITE_BYTES) {
      throw new TypeError(`content exceeds the ${MAX_WRITE_BYTES}-byte write limit.`);
    }
    return {
      path: stringField(value, 'path'),
  ...
  async execute(input, context) {
    ...
    if (existing !== undefined && !input.overwrite) {
      throw new Error(`File already exists: ${input.path}. Use edit_file or set overwrite.`);
    }
    if (existing?.isDirectory() === true) throw new Error(`Path is a directory: ${input.path}`);
...
async function walkFiles(root: string, signal?: AbortSignal): Promise<string[]> {
  ...
  if (!rootStat.isDirectory()) throw new Error(`Not a file or directory: ${root}`);
...
async function searchWithRipgrep(
  ...
  args.push('--glob', '!.git/**', '--glob', '!node_modules/**', '--glob', '!dist/**');
  if (!includeSensitive) {
    args.push(
      '--glob', '!**/.env',
      '--glob', '!**/.env.*',
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code) => {
      ...
      if (code !== 0) return reject(new Error(stderr.trim() || `ripgrep exited with code ${String(code)}`));
...
async function searchWithNode(
  ...
  const matcher = input.pattern === undefined ? undefined : globToRegExp(input.pattern);
  ...
  for (const file of files) {
    ...
    for (const [index, line] of text.split('\n').entries()) {
      const matched = query === undefined ? line.includes(input.query) : query.test(line);
...
function decodeText(buffer: Buffer, label: string): string {
  if (buffer.includes(0)) throw new Error(`Binary file is not supported: ${label}`);
  try {
